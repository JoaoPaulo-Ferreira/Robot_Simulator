...
class Controller:
    ...
    def summary(self):
        ...
        print("TRAVEL TIME      :	", np.around(self.time_total))
    ...
    def UpdateRobotPosition(self, start, next_step):
        while(not self.checkPosition(next_step)):
            ...
            dist = int(polar[0])
            angle = int(polar[1])
            # print(angle - self.robot.theta)
            print(start, next_step, dist, angle)
    ...
    def gerPar(self, dist, angle):
        if(angle == self.robot.theta):
            ...
            # delta teta é positivo ou negativo?
            #  é maior ou menor que 180??
            print("spin")
            vr = self.std_speed
            vl = -self.std_speed
            t = (np.abs(angle - self.robot.theta) * self.robot.w)/np.abs(2*vr)
        return vr, vl, t
                ...
                if(len(self.path) < 2):
                    ...
                    if (not self.robot.is_collision(start,next_step)):
                        ...
                        if(self.checkPosition(next_step)):
                            ...
                            # self.path.pop(0)
                            if(self.checkPosition(self.goal)):
                                self.summary()
                                print("sucess")
                                break                    
                    else:
